...
public class PauseScreen : GameObject
{
    ...
    public PauseScreen()
    {
        ...
        _pauseButton = new Button(new RectangleCollider(new Rectangle(new Point((int) _splashLoc.X - buttonWidth / 2, (int)(_splashLoc.Y + buttonHeight + padding)), new Point(buttonWidth, buttonHeight))), 
            "resume", Color.DarkGreen);
        _quitButton = new Button(new RectangleCollider(new Rectangle(
            new Point((int) _splashLoc.X - buttonWidth / 2, _pauseButton.shape.Y + buttonHeight + padding), new Point(buttonWidth, buttonHeight))), 
            "quit", Color.DarkRed);
    }
    ...
    public override void HandleInput(InputManager inputManager)
    {
        ...
        {
            if (_pauseButton.Contains(target.ToVector2()))
            {
                var gm = GameManager.GetGameManager();
